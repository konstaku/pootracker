...
async function main() {
    ...
    try {
        ...
        await bot.setWebHook(
            'https://poo-tracker-test.onrender.com:443/webhook'
        );
        // Handle webhook
        app.post('/webhook', (req, res) => {
            ...
            if (
                !dialogues.some(dialogue => dialogue.chatId === message.chat.id)
            ) {
                dialogue = new Dialogue(message);
                dialogues.push(dialogue);
                bot.sendMessage(dialogue.chatId, 'New dialogue created!');
            } else {
                dialogue = dialogues.find(
                    dialogue => dialogue.chatId === message.chat.id
                );
            }
